...
<div class="page-setup-section">
  ...
  attachOverrideItemListeners() {
    ...
    if (this._removeOverrideHandler) {
      document.removeEventListener("click", this._removeOverrideHandler);
    }

    this._removeOverrideHandler = (e) => {
      const removeBtn = e.target.closest(".remove-override");
      if (!removeBtn) return;

      const listContainer = removeBtn.closest("#pageBreakOverridesList");
      if (!listContainer) return;

      e.preventDefault();
      e.stopPropagation();

      const pageNum = removeBtn.getAttribute("data-page-num");
      if (confirm(`Remove conditional break override for page ${pageNum}?`)) {
        ...
        listContainer.innerHTML = this.generatePageBreakOverrides();
        setTimeout(() => this.attachOverrideItemListeners(), 100);

        // Apply visual refresh immediately so removed override lines disappear without waiting for Apply.
    ...
    document.addEventListener("click", this._removeOverrideHandler);
  }
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        const pagesInput = document
          .getElementById("newOverridePages")
          ?.value.trim();
        const distance = parseFloat(
          document.getElementById("newOverrideDistance")?.value,
        );
  ...
  applyPageElementsSettings() {
    try {
      ...
      const conditionalBreakEnabled =
        document.getElementById("conditionalPageBreakEnabled")?.checked ||
        false;
      ...
      const pageOverrides = {};
      const overrideItems =
        document.querySelectorAll(".page-break-override-item") || [];
  ...
  insertConditionalPageBreakToPage(pageComponent, pageIndex) {
    ...
    const existingIndicators = pageContentEl.querySelectorAll(
      ".conditional-break-indicator",
    );
    existingIndicators.forEach((ind) => ind.remove());
    const pageNumber = pageIndex + 1;
  ...
  removeAllConditionalPageBreaks() {
    ...
    allPages.forEach((pageComponent) => {
      const pageContent = pageComponent.find(".page-content")[0];
      if (pageContent) {
        const pageContentEl = pageContent.getEl
          ? pageContent.getEl()
          : pageContent.view?.el;
        if (pageContentEl) {
          pageContentEl
            .querySelectorAll('.conditional-break-indicator[data-conditional="true"]')
      ...
      if (mainContentArea) {
        const mainContentEl = mainContentArea.getEl
          ? mainContentArea.getEl()
          : mainContentArea.view?.el;
        if (mainContentEl) {
          mainContentEl
