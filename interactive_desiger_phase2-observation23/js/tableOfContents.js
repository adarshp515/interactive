...
function customTableOfContents(editor) {
  ...
  domc.addType('custom-heading', {
    isComponent(el) {
      if (el.tagName && /^H[1-7]$/.test(el.tagName) && el.classList.contains('heading')) {
    ...
    model: {
      defaults: {
        ...
        level: 'h1',
        styles: `
        h1.heading { font-size: 32px; font-weight: bold; margin: 16px 0; }
        h2.heading { font-size: 24px; font-weight: bold; margin: 14px 0; }
        h3.heading { font-size: 18px; font-weight: bold; margin: 12px 0; }
        h4.heading { font-size: 16px; font-weight: bold; margin: 10px 0; }
        h5.heading { font-size: 14px; font-weight: bold; margin: 8px 0; }
        h6.heading { font-size: 12px; font-weight: bold; margin: 6px 0; }
        h7.heading { font-size: 11px; font-weight: bold; margin: 4px 0; }
      `,
        attributes: {
          class: 'heading',
          'data-i_designer-type': 'text',
      ...
      updateTag() {
        ...
        if (this.view.el) {
          ...
          newEl.className = 'heading';
          newEl.textContent = preservedContent;
          newEl.setAttribute('contenteditable', 'false');
          newEl.setAttribute('data-i_designer-type', 'text');
          newEl.setAttribute('data-i_designer-highlightable', 'true');
    ...
    view: {
      onRender() {
        ...
        this.el.setAttribute('data-i_designer-type', 'text');
        this.el.setAttribute('data-i_designer-highlightable', 'true');
  ...
  editor.Commands.add('generate-toc', {
    run(editor) {
      ...
      const headings = doc.querySelectorAll(
        'h1.heading, h2.heading, h3.heading, h4.heading, h5.heading, h6.heading, h7.heading'
      );
